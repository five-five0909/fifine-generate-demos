					...
					<div class="tn-box-content">
						<div class="tn-tabs">
							...
							<c:if test="${empty jobList }">
									<div
										style="text-align: center; margin: auto; line-height: 100px;">
										<h1>抱歉，您暂未申请任何职位!</h1>
									</div>
								</c:if>
								<c:if test="${!empty jobList }">
											...
											<c:forEach items="${jobList}" var="jobApply">
												<tr class="tn-table-grid-row">
													<td class="tn-width-check"><input type="checkbox"
														class="tn-checkbox tnui-apply" value="${jobApply.applyId }"
														name="applyIdList"></td>
													<td class="tn-width-auto"><a title="${jobApply.job.company.companyName }" 
														href="CompanyServlet?type=select&id=${jobApply.job.company.companyId }"
														target="_blank"> ${jobApply.job.company.companyName }</a></td>
													<th class="tn-width-pic-mini"><a title="${jobApply.job.jobName }"
														href="JobServlet?type=select&jobid=${jobApply.job.jobId}" 
														target="_blank"> ${jobApply.job.jobName }</a></th>
													...
													<td class="tn-width-action2"><span class="tn-action">
															...
															href="JobServlet?type=delete&applyId=${jobApply.applyId }"> <span class="tn-icon it-icon-delete"></span><span
